...
## 3. dbscan for clusters in UMAP
umap.db<-dbscan(res.umap[,9:11], eps = 3.5, minPts =100)
umap.db ## Check if clustering works, else adjust eps and minPts

## Save results and plot
clus<-as.data.frame(umap.db$cluster)
rownames(clus)<-rownames(res.umap)
colnames(clus)<-as.character("cluster")
data.umap.db<-cbind.data.frame(data, clus)
write.table(data.umap.db, file = "3d.umap.db.csv", row.names = F)
bardata=table(data.umap.db$celltype,data.umap.db$cluster)
png("dbsc.3d.umap.png")
barplot(bardata, main = "dbscan of UMAP scores", sub = "eps=3.5, minPts=100", col = c.col, legend.text = T) #, ylim = c(0,100)
dev.off()

## 4. MANOVA of UMAP axes
data<-res.umap
T0in<-subset(data, celltype == "T0in")
T2in<-subset(data, celltype == "T2in")
T0rb<-subset(data, celltype == "T0rb")
T2rb<-subset(data, celltype == "T2rb")
c.in<-rbind(T0in, T2in)
c.rb<-rbind(T0rb, T2rb)

sink("3d.umap.maov.T0vsT2.txt")
in.maov<-manova(cbind(V1, V2, V3) ~ treatment, data = c.in)
print(in.maov)
print(summary(in.maov))

rb.maov<-manova(cbind(V1, V2, V3) ~ treatment, data = c.rb)
print(rb.maov)
print(summary(rb.maov))
sink()